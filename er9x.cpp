...
extern uint8_t DisplayBuf[DISPLAY_W*DISPLAY_H/8] ;
		...
		if (!g_eeGeneral.hideNameOnSplash)
			lcd_putsnAtt(0 * FW, 7 * FH, g_eeGeneral.ownerName,
					sizeof(g_eeGeneral.ownerName), BSS);
		lcd_putsnAtt(1, 4 * FH, TX_name,10, BSS);
...
{
#define SAMPLE_GROUP_SIZE 1

/*
	LDD_ADC_TSample SampleGroup[SAMPLE_GROUP_SIZE];
	uint16_t AnaInput;
	SampleGroup[0].ChannelIdx = 5;
	AD1_CreateSampleGroup(AD1_DeviceData, (LDD_ADC_TSample *) SampleGroup,
			              SAMPLE_GROUP_SIZE);
	AD1_StartSingleMeasurement(AD1_DeviceData);
	while (!AD1_GetMeasurementCompleteStatus(AD1_DeviceData)) {
	};
	AD1_GetMeasuredValues(AD1_DeviceData, (LDD_TData*) &AnaInput);
*/
	BandGap = 240;
}
...
// Clocks every 128 uS
void ISR_TIMER2_OVF_vect(void) //10ms timer
{
/*
  cli();
#ifdef CPUM2561
...
void ISR_TIMER3_CAPT_vect(void)    // capture ppm in 16MHz / 8 = 2MHz
{
  ...
  if (val > 4000 && val < 16000)    // G: Prioritize reset pulse. (Needed when less than 8 incoming pulses)
    ...
    if (lppmInState && lppmInState <= 8) {
      if (val > 800 && val < 2200) {
        ppmInAvailable              = 100;
        g_ppmIns[lppmInState++ - 1] = (int16_t)(val - 1500) * (uint8_t)(g_eeGeneral.PPM_Multiplier + 10) /
